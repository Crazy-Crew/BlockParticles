...
public class ParticleTasks implements ParticleControl {
    ...
    public void playVolcano(final Location location, String id) {
        locations.put(id, this.plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
    ...
    public void playDoubleSpiral(final Location location, String id, Particles particles, int amount) {
        locations.put(id, this.plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
            ...
            int time = 16;
            final Particle particle = particles == Particles.DOUBLE_WITCH ? Particle.SPELL_WITCH : Particle.FIREWORKS_SPARK;
        ...
        locations.put(id, this.plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    //this.plugin.getServer().getScheduler().cancelTask(locations.get(id));
    ...
    public void playSoulWell(final Location location, final String id) {
        ...
        locations.put(id, this.plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
            ...
            void startSoulWell(final Location location, final String id) {
                ...
                S.put(num, plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (loc == 50) loc = 0;
                        if (lifeSpan == 75) {
                            plugin.getServer().getScheduler().cancelTask(S.get(num));
                            S.remove(num);
                        }
                    }
                }, 0, 1));
