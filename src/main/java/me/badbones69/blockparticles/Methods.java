...
public class Methods implements Listener {
    ...
    static Plugin plugin = Bukkit.getServer().getPluginManager().getPlugin("BlockParticlesAPI");
    ...
    private static List<Entity> getNearbyEntities(Location loc, double x, double y, double z) {
        try {
            FallingBlock ent = loc.getWorld().spawnFallingBlock(loc.subtract(0, 2, 0), Material.AIR, (byte) 0);
    ...
    static void startParticles() {
        ...
        for(final String L : BlockParticles.settings.getData().getConfigurationSection("Locations").getKeys(false)) {
            ...
            if(particle.equalsIgnoreCase("Double Witch") || particle.equalsIgnoreCase("DoubleWitch")) PlayParticles.playDoubleSpiral(loc, L, Particle.SPELL_WITCH, 5);
            ...
            if(particle.equalsIgnoreCase("Double Spiral") || particle.equalsIgnoreCase("DoubleSpiral")) PlayParticles.playDoubleSpiral(loc, L, Particle.FIREWORKS_SPARK, 5);
